...
class AuthTokenServiceImpl @Inject() (
   ...
   */
  def create(userID: Long, expiry: FiniteDuration = 10 minutes) = {
    val randLong: Long = new Random().nextLong()
    val token = AuthToken(randLong, userID, clock.now.withZone(DateTimeZone.UTC).plusSeconds(expiry.toSeconds.toInt))
